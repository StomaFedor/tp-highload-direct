...
### Алгоритм подбора рекламы поиска
Подбор рекламы на странице Я.Поиск будет реализован с помощью ключевых слов, фраз и минус-слов:
...
2) С помощью ElasticSearch поиск фраз с этими ключевыми словами, из которых мы по id получаем подходящие рекламные объявления.
3) Ранжируем рекламные объявления (какое показать первым, какое вторым) по стоимости показа рекламного объявления.
  
### Алгоритм подбора рекламы в РСЯ
Подбор рекламы в РСЯ будет осуществляться с помощью интеграции с Я.Браузером:
1) Запрашиваем у Я.Браузера историю поиска пользователя за прошедшую неделю.
2) Ищем по истории поиска ключевые слова и фразы в ElaticSearch.
3) Находим по ad_id связанные рекламы.
4) Ранжируем рекламы по наиболее подходящим (наибольшее количество ключевых слов) и выдаем пользовател.
